...
async function importData() {
  try {
    ...
    if (data.clients) {
      ...
      const { error } = await supabase.from('clients').upsert(clientData, { onConflict: 'firebase_id' })
    ...
    if (data.produits) {
      ...
      const { error } = await supabase.from('produits').upsert(produitData, { onConflict: 'firebase_id' })
    ...
    if (data.techniciens) {
      ...
      const { error } = await supabase.from('techniciens').upsert(techData, { onConflict: 'firebase_id' })
