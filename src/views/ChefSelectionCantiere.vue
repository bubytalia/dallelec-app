...
const accederCantiere = (chantier) => {
  console.log('🔍 Accesso cantiere:', chantier);
  console.log('🔍 Modalita resoconto:', chantier.modalita_resoconto || chantier.modalitaResoconto);
  
  if ((chantier.modalita_resoconto || chantier.modalitaResoconto) === 'percentuale') {
    console.log('📊 Reindirizzo a resoconto-percentuale');
    router.push(`/chef/chantiers/resoconto-percentuale?chantier=${chantier.id}`);
  } else {
    console.log('📏 Reindirizzo a metrages-detail');
