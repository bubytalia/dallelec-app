            ...
            <div v-else>
              <div v-for="demande in demandesEnAttente" :key="demande.id" class="card mb-3">
                <div class="card-body">
                  <div class="row">
                    <div class="col-md-6">
                      <h6>{{ getUserName(demande.userId) }}</h6>
                      <p><strong>Type:</strong> 
                        <span :class="getTypeClass(demande.type)">
                          {{ getTypeLabel(demande.type) }}
                        </span>
                      </p>
                      <p><strong>Période:</strong> {{ formatDate(demande.startDate) }} - {{ formatDate(demande.endDate) }}</p>
                      <p v-if="demande.reason"><strong>Motif:</strong> {{ demande.reason }}</p>
                      <p><strong>Demandé le:</strong> {{ formatDate(demande.createdAt) }}</p>
                    </div>
                ...
                <div class="card-body">
                  <div class="row">
                    <div class="col-md-6">
                      ...
                      <p><strong>Période:</strong> {{ formatDate(demande.startDate) }} - {{ formatDate(demande.endDate) }}</p>
                      <p v-if="demande.reason"><strong>Motif:</strong> {{ demande.reason }}</p>
                      <p><strong>Demande d'annulation le:</strong> {{ formatDate(demande.cancellationRequestedAt) }}</p>
                      <p><strong>Statut original:</strong> 
                        <span :class="getStatusClass(demande.originalStatus || 'pending')">
                          {{ getStatusLabel(demande.originalStatus || 'pending') }}
                        </span>
                      </p>
                    </div>
                    <div class="col-md-6 text-end">
  ...
  if (confirm('Autoriser cette demande de congé ?')) {
    try {
      await updateDoc(doc(db, 'absences', id), {
        status: 'approved',
        approvedAt: new Date(),
        approvedBy: 'admin@dallelec.com'
      });
      
      // Notifica all'utente (qui verrà implementata con Firebase Cloud Messaging)
...
const autoriserAnnulation = async (id) => {
  if (confirm('Autoriser l\'annulation de cette demande de congé ?')) {
    try {
      // Elimina completamente l'assenza
...
const refuserAnnulation = async (id) => {
  if (confirm('Refuser l\'annulation de cette demande de congé ?')) {
    try {
      // Ripristina lo status originale (pending o approved)
      const absence = absences.value.find(a => a.id === id);
      const originalStatus = absence.originalStatus || 'pending';
      
      await updateDoc(doc(db, 'absences', id), {
        status: originalStatus,
        updatedAt: new Date(),
        updatedBy: 'admin@dallelec.com'
      });
