<template>
  <div class="container py-5">
    ...
    <h2 class="text-center mb-4">Clients VIP</h2>
    
    <div v-if="!tableExists" class="alert alert-warning text-center">
      <h5>⚠️ Tabella 'clients_vip' non trovata</h5>
    ...
    <div v-if="tableExists">
      <div class="row mb-3">
        <div class="col">
          <input v-model="newItem.nom" placeholder="Nom / Entreprise" class="form-control" />
        </div>
        <div class="col">
          <input v-model="newItem.prenom" placeholder="Prénom / Contact" class="form-control" />
        </div>
        <div class="col">
          <input v-model="newItem.email" placeholder="Email" class="form-control" type="email" />
        </div>
        <div class="col">
          <select v-model="newItem.client_id" class="form-select">
      ...
      <tbody>
        <tr v-for="item in items" :key="item.id">
          <template v-if="editId === item.id">
            ...
            <td>
              <select v-model="editItem.client_id" class="form-select">
                <option value="">--</option>
                <option v-for="c in clients" :key="c.id" :value="c.id">{{ c.nom }}</option>
              </select>
            </td>
            <td><input type="checkbox" v-model="editItem.actif" /></td>
            <td>
          ...
          <template v-else>
            ...
            <td><span :class="item.actif ? 'badge bg-success' : 'badge bg-danger'">{{ item.actif ? 'Oui' : 'Non' }}</span></td>
            <td>
...
export default {
  ...
  setup() {
    const items = ref([]);
    const clients = ref([]);
    const newItem = ref({ nom: '', prenom: '', email: '', client_id: '' });
    ...
    const fetchItems = async () => {
      const { data, error } = await supabase.from('clients_vip').select('*').order('nom');
      if (!error) {
        items.value = data || [];
    ...
    const addItem = async () => {
      ...
      const payload = { ...newItem.value, actif: true };
      if (!payload.client_id) delete payload.client_id;
      const { error } = await supabase.from('clients_vip').insert([payload]);
      if (!error) {
        newItem.value = { nom: '', prenom: '', email: '', client_id: '' };
        fetchItems();
      } else {
    ...
    const updateItem = async (id) => {
      const { error } = await supabase.from('clients_vip').update(editItem.value).eq('id', id);
      if (!error) { cancelEdit(); fetchItems(); }
    };

    const deleteItem = async (id) => {
      if (!confirm('Confirmer la suppression ?')) return;
      const { error } = await supabase.from('clients_vip').delete().eq('id', id);
      if (!error) fetchItems();
    };
    ...
    return { items, clients, newItem, addItem, editId, editItem, startEdit, cancelEdit, updateItem, deleteItem, tableExists, getClientName };
  }
};
</script>
