    ...
    <div class="card p-4 mb-4">
      ...
      <div class="row">
        <div class="col-md-6 mb-3">
          <label>Client:</label>
          <input v-model="newFacture.clientNom" type="text" class="form-control" />
        </div>
        <div class="col-md-6 mb-3">
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="facture in facturesFiltrees" :key="facture.id">
              ...
              <td>{{ formatCurrency(facture.montantHT) }}</td>
              ...
              <td>{{ facture.dateEcheance ? formatDate(facture.dateEcheance) : '-' }}</td>
              <td>
...
const saveFacture = async () => {
  ...
  try {
    const factureData = {
      ...
      chantierId: newFacture.value.chantierId,
      dateFacture: newFacture.value.dateFacture,
      montantHT: newFacture.value.montantHT,
      tauxTVA: newFacture.value.tauxTVA,
      montantTTC: parseFloat(montantTTCCalcule.value),
      statut: newFacture.value.statut,
      clientNom: newFacture.value.clientNom,
      dateEcheance: newFacture.value.dateEcheance,
      notes: newFacture.value.notes,
      createdAt: new Date()
    };

    if (editingFacture.value) {
      await updateDoc(doc(db, 'factures', editingFacture.value.id), factureData);
