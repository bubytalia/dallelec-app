            ...
            <div class="row">
              ...
              <div class="col-md-6">
                ...
                <p><strong>Devis actuel:</strong> 
                  <span v-if="selectedChantier.devis_id || selectedChantier.devisId" class="badge bg-success">
                    {{ getDevisInfo(selectedChantier.devis_id || selectedChantier.devisId) }}
                  </span>
...
const devisDisponibles = computed(() => {
  return devis.value.filter(d => {
    ...
    const chantierAssocie = chantiers.value.find(c => String(c.devis_id || c.devisId) === String(d.id))
...
const getChantierAssocie = (devisId) => {
  const chantier = chantiers.value.find(c => String(c.devis_id || c.devisId) === String(devisId))
  if (!chantier) return null
  const numero = (chantier.numero_cantiere || chantier.numeroCantiere) ? `N° ${chantier.numero_cantiere || chantier.numeroCantiere} - ` : ''
