...
const addAbsence = async () => {
  const userEmail = localStorage.getItem('userEmail');
  if (!newAbsence.value.type || !newAbsence.value.startDate || !newAbsence.value.endDate || !userEmail) return;
...
const getTypeClass = (type) => {
  switch (type) {
    ...
    case 'jour_ferie': return 'badge bg-primary';
...
const isAbsentToday = () => {
  ...
  return mesAbsences.value.some(absence => 
    ...
    (absence.start_date || absence.startDate) <= today && 
    (absence.end_date || absence.endDate) >= today
  );
};
