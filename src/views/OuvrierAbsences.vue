...
const fetchAbsences = async () => {
  ...
  try {
    const q = query(collection(db, 'absences'), where('userId', '==', currentUser.value.email));
...
const addAbsence = async () => {
  if (!newAbsence.value.type || !newAbsence.value.startDate || !newAbsence.value.endDate || !currentUser.value) return;
