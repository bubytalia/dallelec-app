<template>
  <div class="container py-4">
    ...
    <h2 class="text-center mb-4">Resoconto Percentuale (Admin)</h2>
          ...
          <tr v-for="zona in zones" :key="zona">
            ...
            <td>
              ...
              <router-link 
                ...
                :to="`/admin/resoconto-finale?chantier=${selectedChantierId}&zone=${zona}`"
    ...
    <div v-if="showHistorique" class="card p-4 mb-4">
      ...
      <table class="table table-striped">
        <thead>
          <tr>
            <th>Date</th><th>Période</th><th>Description</th><th>Zones modifiées</th><th>Statut</th><th>Actions</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="resoconto in historiqueResoconti" :key="resoconto.id">
            ...
            <td>
              <span class="badge" :class="resoconto.status === 'approved' ? 'bg-success' : 'bg-primary'">
                {{ resoconto.status === 'approved' ? 'Approuvé' : resoconto.status }}
              </span>
            </td>
            <td>
...
const loadChantierData = async () => {
  ...
  if (devisId) {
    try {
      const { data: devisDataResult, error } = await supabase.from('devis').select('*').eq('id', devisId).single();
      if (error) throw error;
      
      if (devisDataResult) {
        nomClient.value = devisDataResult.nom || '';
        if (devisDataResult.produits?.length > 0) {
          const zoneSet = new Set();
          devisDataResult.produits.forEach(p => { if (p.zone) zoneSet.add(p.zone); });
          zones.value = Array.from(zoneSet).sort();
          zones.value.forEach(zona => {
            if (!avancementiMensili.value[zona]) avancementiMensili.value[zona] = 0;
          });
        }
      }
    } catch (error) {
...
const loadAvancementiStorici = async () => {
  try {
    ...
    zones.value.forEach(zona => { avancementiStorici.value[zona] = 0; });
...
const loadZoneConvertite = async () => {
  try {
    const { data } = await supabase.from('zone_convertite').select('*').eq('chantier_id', selectedChantierId.value);
    zoneConvertite.value = data ? new Set(data.map(item => item.zona)) : new Set();
  } catch (error) {
    zoneConvertite.value = new Set();
  }
};

const getAvancementPrecedent = (zona) => Math.min(avancementiStorici.value[zona] || 0, 100);
const getTotalAvancement = (zona) => Math.min((getAvancementPrecedent(zona)) + (avancementiMensili.value[zona] || 0), 100);
const isZoneCompleta = (zona) => getAvancementPrecedent(zona) >= 100;
const isZoneConvertita = (zona) => zoneConvertite.value.has(zona);
...
const zoneCompleteDaConvertire = computed(() => 
  zones.value.filter(zona => getTotalAvancement(zona) === 100 && !isZoneConvertita(zona))
);
...
const sauvegarderResoconto = async () => {
  try {
    ...
    alert(`✅ Resoconto sauvegardé!\n\nPériode: ${periodeMonth.value}\nZones: ${zoneModificate}`);
    
    zones.value.forEach(zona => { avancementiMensili.value[zona] = 0; });
    descrizione.value = '';
    regies.value = [];
...
const voirHistorique = async () => {
  ...
  if (showHistorique.value) {
    try {
      const { data, error } = await supabase.from('resoconti_percentuali').select('*')
        .eq('chantier_id', selectedChantierId.value).order('created_at', { ascending: false });
      if (error) throw error;
...
const supprimerResoconto = async (id) => {
  if (confirm('Supprimer ce resoconto?')) {
    try {
      const { error } = await supabase.from('resoconti_percentuali').delete().eq('id', id);
      if (error) throw error;
...
const convertirZone = async (zona) => {
  if (confirm(`Convertir la zone "${zona}" en métrage détaillé?`)) {
    try {
      const { error } = await supabase.from('zone_convertite').insert([{
        chantier_id: selectedChantierId.value, zona: zona, convertita_il: new Date().toISOString()
      }]);
      if (error) throw error;
      router.push(`/admin/metrages-detail?chantier=${selectedChantierId.value}&zona=${zona}&conversion=true`);
    } catch (error) {
      alert('Erreur: ' + error.message);
    }
  }
